...
- 普通编码任务直接修改源码，并按任务需要执行不依赖服务器的检查或测试；除非用户明确要求构建/打包，否则不必为了保持 `dist/` 最新而自动运行构建。
- 只有用户明确要求启动、停止或重启服务器时，才操作 `lan-ai-server`、联机大厅、开发服务器或预览服务器。不得仅为编码或验证而自动关服、开服或访问服务地址。
...
2. 普通游戏内容修改只改源码；仅当用户明确要求更新构建产物或打包时，才重新构建并让根目录 `dist/` 保持最新。
3. 只有用户明确要求启动、停止或重启 `lan-ai-server` 时，才操作该服务并按需验证 `http://127.0.0.1:8090/`；其他任务不要自动处理服务生命周期。
...
1. 用户明确要求生成新的 Windows 安装包/客户端时，先更新源码并运行 `pnpm build`，再生成客户端。

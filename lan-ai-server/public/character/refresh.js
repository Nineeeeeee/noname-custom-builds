...
const skills = {
  ...
  reqice: {
    ...
    filter(event, player) {
      ...
      return lib.inpile.some((name) => {
        ...
        const card = get.autoViewAs({ name }, hs2);
  ...
  rezhanjue: {
    ...
    filter(event, player) {
      ...
      for (var i = 0; i < hs2.length; i++) {
        var mod2 = game.checkMod(hs2[i], player, "unchanged", "cardEnabled2", player);
        if (mod2 === false) {
          return false;
        }
      }
      return event.filterCard(get.autoViewAs({ name: "juedou" }, hs2));
    },
    viewAs: { name: "juedou" },
    onuse(links, player) {
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          ...
          let hs2 = player.getCards("h"), val = hs2.reduce((acc, i) => acc - get.value(i, player), 0) / 6 + 1;
          if (td > 0) {
            return val;
          }
          if (player.hasSkillTag("directHit_ai", true, {
            target,
            card: get.autoViewAs({ name: "juedou" }, hs2)
          })) {
            return val;
          }
          ...
          let damage = pd / get.attitude(player, player), ps = player.mayHaveSha(player, "respond", hs2, "count");
          if (att > 0) {
            if (ts < 1) {
              return val;
            }
        ...
        target(player, target) {
          ...
          let ts = target.mayHaveSha(player, "respond", null, "count"), ps = player.mayHaveSha(player, "respond", hs2, "count");
          if (ts < 1) {
            return td + 1;
          }
          if (pd >= 0) {
    ...
    async content(event, trigger, player) {
      ...
      if (bool) {
        ...
        result = await target.chooseControl().set("choiceList", choiceList).set("prompt", "安东：请选择一项").set("ai", function(event2, target2) {
          ...
          const hs2 = target2.getGainableCards(player2, "h", (card) => get.suit(card) == "heart");
          if (hs2.length) {
            const recover = hs2.filter((card) => get.tag(card, "save") || get.tag(card, "recover")).length;
      ...
      effect: {
        target(card, player, target) {
          ...
          if (get.tag(card, "damage") && player != target && get.attitude(player, target) < 0) {
            ...
            const recover = hs.filter((card2) => get.tag(card2, "save") || get.tag(card2, "recover")).length;
  ...
  rezhuhai: {
    ...
    async content(event, trigger, player) {
      ...
      var bool = false, hs2 = player.getCards("h");
      for (var i of hs2) {
    ...
    filterTarget(card, player, target) {
      ...
      if (!ui.selected.targets.length) {
        ...
        return game.hasPlayer(function(current2) {
          ...
          return (hs2 > 0 || cs2 > 0) && Math.abs(hs2 - cs2) <= ps;
        });
      }
      var current = ui.selected.targets[0], hs2 = target.countCards("h"), cs = current.countCards("h");
      return (hs2 > 0 || cs > 0) && Math.abs(hs2 - cs) <= ps;
    },
    multitarget: true,
    multiline: true,
    async content(event, trigger, player) {
  ...
  xinquanji: {
    ...
    async content(event, trigger, player) {
      ...
      const result = hs2.length == 1 ? { bool: true, cards: hs2 } : await player.chooseCard("h", true, "选择一张手牌作为“权”").forResult();
      if (result?.bool && result?.cards?.length) {
  ...
  rehuaiyi: {
    ...
    async content(event, trigger, player) {
      await player.showHandcards();
      const hs2 = player.getCards("h"), color = get.color(hs2[0], player);
      if (hs2.length === 1 || !hs2.some((card, index) => {
  ...
  decadexuanfeng: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(
        ...
        let player2 = get.event().player, att = get.attitude(player2, target), hs2 = target.countCards("h"), es = target.countCards("e");
        if (hs2 && target.hasSkillTag("noh") || es && target.hasSkillTag("noe")) {
          att *= 0.8;
        } else {
          att = -att;
        }
  ...
  hanzhan: {
    ...
    async content(event, trigger, player) {
      ...
      for (const target of targets) {
        const hs2 = target.getCards("h");
        if (hs2.length) {
          trigger.fixedResult[target.playerid] = hs2.randomGet();
        }
      }
    },
    group: "hanzhan_gain",
    ...
    ai: {
      result: {
        target(player, target) {
          ...
          var bool = false;
          for (var i = 0; i < hs2.length; i++) {
            if (get.number(hs2[i]) >= 9 && get.value(hs2[i]) < 7) {
              bool = true;
              break;
            }
          }
